...
def xml_to_yolo(folders, eval_folders, yolo_dir, names_to_labels):
	'''
	This function creates a Yolo-ready database containing images and TXT files in the same
	directory. TXT files are yolo-style encoded.

	FOLDERS - List of absolute paths to folders containing train or test images and XML files.

	YOLO_DIR - Directory where images and TXT files will be saves.

	NAMES_TO_LABELS - Dictionary with object name as key and label number as value. This makes
	explicit the relation object-label contrary to a list.
	'''
	print('Creating yolo database...')
	...
	# Stuff to count things and to allow unique names
	count      = dict.fromkeys(names_to_labels.keys(), 0)
	img_count  = -1
	identifier = 'im_'

	for folder in folders + eval_folders:
		for file in os.listdir(folder):
			# List only XML files
			if(os.path.splitext(file)[1] != '.xml'): continue
			...
			# Extract information
			xml_file_path = tree.find('path').text
			size          = tree.find('size')
			width         = size.find('width').text
			height        = size.find('height').text

			# Extract and format data
			(root, ext)  = os.path.splitext(os.path.join(xml_file_path))
			...
			txt_file_path = os.path.join(save_dir, identifier + str(img_count)) + '.txt'
			img_file_dir  = root + '.jpg'
			new_img_path  = os.path.join(save_dir, tail) + '.jpg'
			new_name      = os.path.join(save_dir, identifier + str(img_count)) + '.jpg'

			# Copy images to YOLO dir and give an unique name
			if os.path.isfile(img_file_dir) and not os.path.isfile(new_name):
				shutil.copy(img_file_dir, save_dir)
				os.rename(new_img_path, new_name)

			# Create TXT file with bndbox information extracted from XML files
			with open(txt_file_path, 'w') as txt_file:
				for obj in tree.findall('object'):
					name = obj.find('name').text
					if name in names_to_labels.keys():
						count[name] += 1

					# Only keep objects used for training
					if name not in names_to_labels.keys(): continue

					# Retreive VOC style data
					xmin = obj.find('bndbox').find('xmin').text
					ymin = obj.find('bndbox').find('ymin').text
					xmax = obj.find('bndbox').find('xmax').text
					ymax = obj.find('bndbox').find('ymax').text

					# COmpute YOLO-style data
					x = (float(xmax) + float(xmin)) / float(width) / 2
					y = (float(ymax) + float(ymin)) / float(height) / 2
					w = (float(xmax) - float(xmin)) / float(width)
					h = (float(ymax) - float(ymin)) / float(height)

					# Write file
					line = '{} {} {} {} {}\n'.format(str(names_to_labels[name]), str(x), str(y), str(w), str(h))
					txt_file.write(line)

	with open(train_file, 'w') as test_f:
		for file in os.listdir(train_dir):
			if os.path.splitext(file)[1] != '.jpg': continue

			name     = os.path.split(file)[1]
			new_name = os.path.join('data/train/', name)
			test_f.write(new_name + '\n')

	with open(val_file, 'w') as val_f:
		for file in os.listdir(val_dir):
			if os.path.splitext(file)[1] != '.jpg': continue
			name     = os.path.split(file)[1]
			new_name = os.path.join('data/val/', name)
			val_f.write(new_name + '\n')

	# Show stuff
	print('done!')
	print("Nb images: {}".format(img_count + 1))
	print('Nb bbox: {}'.format(sum(count.values())))
	for label, nb in count.items():
		print('  {}: {}'.format(label, nb))


def create_yolo_train_txt(image_folder, file_path):
	'''
	Creates TXT file for YOLO training. This function assumes JPG images and specific path structures.

	IMAGE_FOLDER - Folder containing images for YOLO training. This folder is the same
	as YOLO_DIR populated by xml_to_yolo() func.

	FILE_PATH - Directory where YOLO training TXT file will be saved.
	'''
	print('Creating TXT train files...')

	(_, tail) = os.path.split(image_folder)
	...
	with open(file_path, 'w') as text_file:
		for file in os.listdir(image_folder):
			...
			# Retreive image path and copy it to the output TXT file
			name = os.path.split(file)[1]
			file_name = os.path.join('data/', tail, name)
...
def xml_to_yolo_2(folders, yolo_dir, names_to_labels, ratio=0.8):
	'''
	This function creates a Yolo-ready database containing images and TXT files in the same
	directory. TXT files are yolo-style encoded.
	...
	# Stuff to count things and to allow unique names
	plant_count = dict.fromkeys(names_to_labels.keys(), 0)
	tr_val_cnt  = {'train': 0, 'val': 0}
	img_count   = -1
	identifier  = 'im_'
	xml_list    = []
	...
	for folder in folders:
		for file in os.listdir(folder):
			# List only XML files
			if(os.path.splitext(file)[1] == '.xml'):
				xml_list.append(os.path.join(folder, file))

	shuffle(xml_list)
	nb_train_samples = int(ratio * len(xml_list))
	tr_val_cnt['train'] = nb_train_samples
	tr_val_cnt['val']   = len(xml_list) - nb_train_samples

	for index, element in enumerate(xml_list):
		# Parse XML tree
		tree = ET.parse(element).getroot()
	...
	print('done!')
	print("Nb images: {}".format(img_count + 1))
	for element, amount in tr_val_cnt.items():
		print('  {}: {}'.format(element, amount))
	print('Nb bbox: {}'.format(sum(plant_count.values())))
	for label, nb in plant_count.items():
		print('  {}: {}'.format(label, nb))
...
def main(args=None):
	...
	eval = Evaluator()
	eval.getAreaMetrics(boundingBoxes)
