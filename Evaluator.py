...
class Evaluator:
    ...
    def getAreaMetrics(self, boundingboxes, IOUThreshold=0.5, method=MethodAveragePrecision.EveryPointInterpolation):
        ...
        for c in classes:
            ...
            TP = np.zeros(len(dects))
            FP = np.zeros(len(dects))
            area = np.zeros(len(dects))
            IoUs = np.zeors(len(dects))
            # create dictionary with amount of gts for each image
            det = Counter([cc[0] for cc in gts])
            print(det)
            for key, val in det.items():
